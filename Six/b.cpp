...
pthread_t enterRope(int id, Destination destination, SharedMemory *sharedMemory) {
...
void* monkeyConsumer(void *sharedMemory) {
    ...
    Destination rrCurrentDestination = east;
        ...
        if (memory->criticalSection == 1) {
            if (rrCurrentDestination == east && memory->numOfEastwardMonkeys > 0 && memory->westwardMutex == 1 &&
                ...
                while (counter < memory->rrQuantum) {
                    if (memory->numOfEastwardMonkeys == 0) {
                        break;
                    } else {
                        Monkey eastwardMonkey = removeEastwardMonkey((memory));
                        memory->monkeysOnRope.push(eastwardMonkey);
                        tidMonkeys[counter] = enterRope(eastwardMonkey.id, eastwardMonkey.destination, memory);
                        counter++;
                    }
                }
                for (int i = 0; i < counter; ++i) {
                    pthread_join(tidMonkeys[i], NULL);
                }
                if (memory->monkeysOnRopeSemaphore == 0) {
                    release(memory, eastwardMut);
                }
                ...
                pthread_t tidMonkeys[memory->rrQuantum];
                while (counter < memory->rrQuantum) {
                    if (memory->numOfWestwardMonkeys == 0) {
                        break;
                    } else {
                        Monkey westwardMonkey = removeWestwardMonkey(memory);
                        memory->monkeysOnRope.push(westwardMonkey);
                        tidMonkeys[counter] = enterRope(westwardMonkey.id, westwardMonkey.destination, memory);
                        counter++;
                    }
                }
                for (int i = 0; i < counter; ++i) {
                ...
                if (memory->monkeysOnRopeSemaphore == 0) {
                    release(memory, westwardMut);
                }
                rrCurrentDestination = east;
            }
            release(memory, criticalSection);
        }
    }
